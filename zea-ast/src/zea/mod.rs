...
impl PrettyAST for StatementBlock {
    fn pretty_print(&self, depth: usize) -> String {
        let mut buffer = format!("BLOCK {}", self.id).pretty_print(depth);

        for s in self.statements.iter() {
            buffer += &format!("BLOCKSTMT {}", s.id).pretty_print(depth + 1);
            buffer += &s.pretty_print(depth + 1);
            buffer += &format!("/BLOCKSTMT {}", s.id).pretty_print(depth + 1);
        }
        buffer += &format!("/BLOCK {}", self.id).pretty_print(depth);
...
impl PrettyAST for Statement {
    fn pretty_print(&self, depth: usize) -> String {
        match &self.kind {
            ...
            StatementKind::CondBranch(b) => b.pretty_print(depth),
            StatementKind::ExpandedBlock(eb) => eb.pretty_print(depth),
            StatementKind::FunctionCall(c) => c.pretty_print(depth),
...
impl PrettyAST for IfThenElse {
    fn pretty_print(&self, depth: usize) -> String {
        let mut buffer = "IFELSE".pretty_print(depth);

        buffer += &"#COND".pretty_print(depth + 1);
        buffer += &self.condition.pretty_print(depth + 2);

        buffer += &"#IFTRUE".pretty_print(depth + 1);
        buffer += &self.true_case.pretty_print(depth + 2);

        if let Some(e) = self.false_case.as_ref() {
            buffer += &"#FALSECASE".pretty_print(depth + 1);
            buffer += &e.pretty_print(depth + 2);
        };
        buffer
    }
}
...
impl PrettyAST for Expression {
    fn pretty_print(&self, depth: usize) -> String {
        ...
        match &self.kind {
            ...
            ExpressionKind::CondBranch(b) => b.pretty_print(depth),
            ExpressionKind::Block(b) => b.pretty_print(depth),
            ExpressionKind::ExpandedBlock(eb) => eb.pretty_print(depth),
            ExpressionKind::FuncCall(c) => c.pretty_print(depth),
            ExpressionKind::Unit => "UNITVALUE".pretty_print(depth),
            _ => todo!("pretty print expression of kind {:?}", self.kind),
        }
    }
}
impl PrettyAST for FunctionCall {
    fn pretty_print(&self, depth: usize) -> String {
        let mut buffer = "CALL".pretty_print(depth);

        buffer += &"#FUNC".pretty_print(depth + 1);
        buffer += &self.name.pretty_print(depth + 2);

        if !self.args.is_empty() {
            for arg in self.args.iter() {
                buffer += &"#ARG".pretty_print(depth + 1);
                buffer += &arg.pretty_print(depth + 2);
            }
        };
        buffer
    }
